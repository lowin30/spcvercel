...
export function TaskStatusBadge({ task }) {
  ...
  if (task.estado_tarea) {
    ...
    if (task.id_estado_nuevo) {
      ...
      const idEstado = parseInt(task.id_estado_nuevo);
      if (ESTADOS_MAP[idEstado]) {
        estadoColor = ESTADOS_MAP[idEstado].color;
      }
    }
  ...
  // Por último usar id_estado_nuevo y mapearlo a un nombre
    ...
    if (ESTADOS_MAP[idEstado]) {
      estadoNombre = ESTADOS_MAP[idEstado].nombre;
      estadoColor = ESTADOS_MAP[idEstado].color;
    } else {
  ...
  if (estadoNombre && !estadoColor) {
    ...
    Object.keys(ESTADOS_MAP).forEach(key => {
      if (ESTADOS_MAP[key].nombre.toLowerCase() === estadoNombre.toLowerCase()) {
        estadoColor = ESTADOS_MAP[key].color;
      }
    });
  }
